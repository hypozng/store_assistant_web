<template>
  <el-dialog :title="`字典信息${title}`" :visible.sync="visible" :close-on-click-modal="false" :append-to-body="false" :modal-append-to-body="false" width="500px">
    <el-form ref="form" :model="formData" size="medium" label-width="120px">
      <ff-input label="分组" prop="groupKey" v-model="formData.groupKey" :disabled="local" />
      <ff-input label="名称" prop="name" v-model="formData.name" />
      <ff-input v-if="local" label="编码" prop="code" v-model="formData.code" />
      <ff-textarea label="值" prop="value" v-model="formData.value" />
